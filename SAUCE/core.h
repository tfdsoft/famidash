...
// Banked Famitone2 calls
#define FAMITONE_MUSIC_INIT(SONG_DATA) ( \
  mmc3_bank_num = music_bank, \
  mmc3_push_data_bank(), \
  famitone_init((void*)SONG_DATA), \
  mmc3_pop_data_bank() \
)
#define FAMITONE_SFX_INIT(SFX_DATA) ( \
  mmc3_bank_num = sfx_bank, \
  mmc3_push_data_bank(), \
  sfx_init((void*)SFX_DATA), \
  mmc3_pop_data_bank() \
)
#define FAMITONE_MUSIC_PLAY(SONG) ( \
  mmc3_bank_num = music_bank, \
  mmc3_push_data_bank(), \
  music_play(SONG), \
  mmc3_pop_data_bank() \
)
#define FAMITONE_SFX_PLAY(SFX, CHANNEL) ( \
  mmc3_bank_num = sfx_bank, \
  mmc3_push_data_bank(), \
  sfx_play(SFX, CHANNEL), \
  mmc3_pop_data_bank() \
)
#define FAMITONE_DPCM_PLAY(SAMPLE) ( \
  mmc3_bank_num = dpcm_bank, \
  mmc3_push_data_bank(), \
  sample_play(SAMPLE), \
  mmc3_pop_data_bank() \
)
...
// Push code bank
#define PUSH_CODE_BANK(_BANK) do { \
  mmc3_bank_num = (_BANK); \
  mmc3_push_code_bank(); \
} while(0)

// Pop code bank
#define POP_CODE_BANK() do { \
  mmc3_pop_code_bank(); \
} while(0)

// Banked call
#define CALL(_BANK, _FN) do { \
  mmc3_bank_num = (_BANK); \
  mmc3_push_code_bank(); \
  _FN(); \
  mmc3_pop_code_bank(); \
...
// Banked call with args
#define CALL_WITH_ARGS(_BANK, _FN, ...) do { \
  mmc3_bank_num = (_BANK); \
  mmc3_push_code_bank(); \
  _FN(__VA_ARGS__); \
  mmc3_pop_code_bank(); \
...
// Banked call with returned value
#define R_CALL(_BANK, _FN, _RV) do { \
  mmc3_bank_num = (_BANK); \
  mmc3_push_code_bank(); \
  _RV = _FN(); \
  mmc3_pop_code_bank(); \
...
// Banked call with args and returned value
#define R_CALL_WITH_ARGS(_BANK, _FN, _RV, ...) do { \
  mmc3_bank_num = (_BANK); \
  mmc3_push_code_bank(); \
  _RV = _FN(__VA_ARGS__); \
  mmc3_pop_code_bank(); \
