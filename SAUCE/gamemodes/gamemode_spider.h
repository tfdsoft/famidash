...
void spider_movement(){
	...
	if (!currplayer_gravity) {
		if(((controllingplayer->a || controllingplayer->up) || cube_data[currplayer] & 2) && currplayer_vel_y == 0 && !orbed[currplayer] && !dashing[currplayer] && !spidered[currplayer]) {
			idx8_store(cube_data, currplayer, cube_data[currplayer] & 0b11111101);	
			jumps++;
			spidered[currplayer] = 1;
		...
		else if (!(controllingplayer->a || controllingplayer->up)) spidered[currplayer] = 0;
	}	
	else {
		if(((controllingplayer->a || controllingplayer->up) || cube_data[currplayer] & 2) && currplayer_vel_y == 0 && !orbed[currplayer] && !dashing[currplayer] && !spidered[currplayer]) {
			idx8_store(cube_data, currplayer, cube_data[currplayer] & 0b11111101);
