...
def binpack_and_write_data(bg_exp_data : tuple, spr_exp_data : tuple, include_path : pathlib.Path):
	...
	if len(bg_exp_data[0] + spr_exp_data[0]) > 0:
...
def main():
	...
	filteredOfficialMetadata = filterMetadata(metadata.get('official_levels', []))
	filteredCommunityMetadata = filterMetadata(metadata.get('community_levels', []))
	filteredMetadata = filteredOfficialMetadata + filteredCommunityMetadata
	levels = [i['level'] for i in filteredMetadata]

	globalObjectOffsetSettings = metadata.get('globalObjectOffsets', [])

	levelSetDefineName = metadata['levelSetDefineName']	# This is a must have
