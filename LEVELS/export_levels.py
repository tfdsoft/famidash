...
include_path = (own_path / "include").resolve()
...
def generate_menutext(filteredMetadata : dict):
...
def generate_level_header(filteredMetadata : dict):
	output = [
...
def main():
	...
	generate_menutext(filteredMetadata)
	generate_level_list(filteredMetadata)
	generate_level_header(filteredMetadata)

	mid_widths_enabled = max(bg_exp_data[1]) >= 0x100
	hi_widths_enabled = max(bg_exp_data[1]) >= 0x10000
	
	with open(include_path.joinpath("all_level_table.s"), 'w') as out:
		level_list_lo = '\n'.join(
