
// Exported by export_levels.py

const char levelText00[ 7] = "LOOK AT";
const char levelText01[ 5] = "FOFII";
const char levelText02[10] = "GENERATION";
const char levelText03[ 7] = "SCARLET";
const char levelText04[ 7] = "RAINBOW";
const char levelText05[12] = "DORABAEBASIC";
const char levelText06[ 6] = "SUBTLE";
const char levelText07[ 8] = "DANCE OF";
const char levelText08[ 8] = "A PRETTY";
const char levelText09[ 8] = "SKELETAL";
const char levelText0A[ 8] = "INFINITE";
const char levelText0B[ 5] = "SONAR";
const char levelText0C[13] = "DARK PARADISE";
const char levelText0D[ 5] = "NINOX";
const char levelText0E[10] = "THIS LEVEL";
const char levelText0F[ 9] = "MOONLIGHT";
const char levelText10[11] = "FOFII FOFII";
const char levelText11[10] = "PYROPHORIC";
const char levelText12[ 9] = "CRYOGENIC";
const char levelText13[ 6] = "OCEANE";
const char levelText14[ 5] = "RETRO";
const char levelText15[ 5] = "SURGE";
const char levelText16[10] = "TINY TUNES";
const char levelText17[ 9] = "FIRE AURA";
const char levelText18[ 4] = "DUST";
const char levelText19[12] = "FACTORY TIME";
const char levelText1A[14] = "THE STEAMWORKS";
const char levelText1B[ 2] = "10";
const char levelText1C[ 8] = "ODDITIES";
const char levelText1D[10] = "POWER TRIP";
const char levelText1E[ 7] = "VIOLINS";
const char levelText1F[10] = "EASY LEVEL";
const char levelText20[ 9] = "HIGH LIFE";
const char levelText21[11] = "SHENANIGANS";
const char levelText22[11] = "PROBLEMATIC";
const char levelText23[ 6] = "DECODE";
const char levelText24[ 7] = "CIRCLES";
const char levelText25[10] = "DEATH MOON";
const char levelText26[13] = "CLUTTERFUNK 2";
const char levelText27[11] = "SPEED RACER";
const char levelText28[ 4] = "HELL";
const char levelText29[11] = "FUTURE FUNK";
const char levelText2A[10] = "LUCKY DRAW";


const char* const levelTextsUpper[] = {
	NULL,
	NULL,
	NULL,
	levelText00,
	NULL,
	levelText01,
	NULL,
	NULL,
	NULL,
	levelText02,
	levelText03,
	NULL,
	NULL,
	levelText04,
	NULL,
	NULL,
	levelText05,
	levelText06,
	NULL,
	levelText07,
	levelText08,
	NULL,
	levelText09,
	NULL,
	NULL,
	levelText0A,
	NULL,
	NULL,
	NULL,
	NULL,
	NULL,
	NULL,
};

const uint8_t levelTextsUpperSize[] = {
	0,
	0,
	0,
	sizeof(levelText00),
	0,
	sizeof(levelText01),
	0,
	0,
	0,
	sizeof(levelText02),
	sizeof(levelText03),
	0,
	0,
	sizeof(levelText04),
	0,
	0,
	sizeof(levelText05),
	sizeof(levelText06),
	0,
	sizeof(levelText07),
	sizeof(levelText08),
	0,
	sizeof(levelText09),
	0,
	0,
	sizeof(levelText0A),
	0,
	0,
	0,
	0,
	0,
	0,
};


const char* const levelTextsLower[] = {
	levelText0B,
	levelText0C,
	levelText0D,
	levelText0E,
	levelText0F,
	levelText10,
	levelText11,
	levelText12,
	levelText13,
	levelText14,
	levelText15,
	levelText16,
	levelText17,
	levelText18,
	levelText19,
	levelText1A,
	levelText1B,
	levelText1C,
	levelText1D,
	levelText1E,
	levelText1F,
	levelText20,
	levelText21,
	levelText22,
	levelText23,
	levelText24,
	levelText25,
	levelText26,
	levelText27,
	levelText28,
	levelText29,
	levelText2A,
};

const uint8_t levelTextsLowerSize[] = {
	sizeof(levelText0B),
	sizeof(levelText0C),
	sizeof(levelText0D),
	sizeof(levelText0E),
	sizeof(levelText0F),
	sizeof(levelText10),
	sizeof(levelText11),
	sizeof(levelText12),
	sizeof(levelText13),
	sizeof(levelText14),
	sizeof(levelText15),
	sizeof(levelText16),
	sizeof(levelText17),
	sizeof(levelText18),
	sizeof(levelText19),
	sizeof(levelText1A),
	sizeof(levelText1B),
	sizeof(levelText1C),
	sizeof(levelText1D),
	sizeof(levelText1E),
	sizeof(levelText1F),
	sizeof(levelText20),
	sizeof(levelText21),
	sizeof(levelText22),
	sizeof(levelText23),
	sizeof(levelText24),
	sizeof(levelText25),
	sizeof(levelText26),
	sizeof(levelText27),
	sizeof(levelText28),
	sizeof(levelText29),
	sizeof(levelText2A),
};
