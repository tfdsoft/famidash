; This file is for the FamiStudio Sound Engine and was generated by FamiStudio
; Required flags for Famidash music:
; FAMISTUDIO_USE_FAMITRACKER_TEMPO = 1
; FAMISTUDIO_USE_FAMITRACKER_DELAYED_NOTES_OR_CUTS = 1
; FAMISTUDIO_USE_RELEASE_NOTES = 1
; FAMISTUDIO_USE_VOLUME_TRACK = 1
; FAMISTUDIO_USE_VOLUME_SLIDES = 1
; FAMISTUDIO_USE_PITCH_TRACK = 1
; FAMISTUDIO_USE_SLIDE_NOTES = 1
; FAMISTUDIO_USE_VIBRATO = 1
; FAMISTUDIO_USE_DUTYCYCLE_EFFECT = 1
; FAMISTUDIO_USE_DPCM_BANKSWITCHING = 1

.export _music_data_famidash_music8:=music_data_famidash_music8

music_data_famidash_music8:
	.byte 1
	.word @instruments
	.word @samples-5
; 00 : Deep Swim
	.word @song0ch0
	.word @song0ch1
	.word @song0ch2
	.word @song0ch3
	.word @song0ch4
	.word 249,208
; The DPCM aligner used to be here

.export music_data_famidash_music8
.global FAMISTUDIO_DPCM_PTR

@instruments:
	.word @env19,@env3,@env1,@env0 ; 00 : cymbal
	.word @env11,@env3,@env16,@env0 ; 01 : rel hats
	.word @env6,@env12,@env1,@env0 ; 02 : snare 2
	.word @env7,@env18,@env16,@env0 ; 03 : kick
	.word @env5,@env3,@env17,@env0 ; 04 : hat
	.word @env10,@env13,@env16,@env0 ; 05 : blank
	.word @env15,@env13,@env16,@env0 ; 06 : infernoplex_piano
	.word @env20,@env13,@env16,@env0 ; 07 : blank slower string loud
	.word @env15,@env13,@env2,@env0 ; 08 : infernogame_lead_piano
	.word @env14,@env3,@env1,@env0 ; 09 : crash mini

@env0:
	.byte $00,$c0,$7f,$00,$02
@env1:
	.byte $c1,$c0,$00,$01
@env2:
	.byte $c2,$c2,$c0,$00,$00
@env3:
	.byte $bd,$c0,$00,$01
@env4:
	.byte $00,$c0,$b6,$af,$ab,$ac,$b2,$bb,$c5,$ce,$d4,$d5,$d1,$ca,$00,$01
@env5:
	.byte $00,$c8,$c4,$c2,$c0,$00,$04
@env6:
	.byte $00,$cd,$cb,$c8,$c8,$c7,$c7,$c6,$c7,$c7,$c5,$c4,$c3,$c2,$c1,$00,$0e
@env7:
	.byte $00,$cd,$c9,$c5,$c2,$c1,$c0,$00,$06
@env8:
	.byte $00,$c0,$bf,$03,$c0,$c0,$c1,$02,$c1,$00,$01
@env9:
	.byte $00,$cf,$7f,$00,$02
@env10:
	.byte $04,$cf,$00,$01,$ca,$c6,$c4,$c2,$c0,$00,$08
@env11:
	.byte $00,$c8,$c6,$c5,$c5,$c4,$c4,$c3,$02,$c2,$02,$c1,$02,$c0,$00,$0d
@env12:
	.byte $c8,$ce,$00,$01
@env13:
	.byte $c0,$7f,$00,$01
@env14:
	.byte $22,$cf,$04,$ce,$ce,$cd,$cd,$cc,$03,$cb,$03,$ca,$03,$c9,$02,$c8,$03,$c7,$03,$c6,$03,$c5,$02,$c4,$04,$c3,$06,$c2,$09,$c1,$11,$c1,$00,$1f,$c1,$c0,$00,$23
@env15:
	.byte $00,$ce,$02,$cd,$cd,$cc,$cc,$cb,$cb,$ca,$c9,$c9,$c8,$c7,$c6,$c5,$c3,$00,$10
@env16:
	.byte $7f,$00,$00
@env17:
	.byte $c0,$c1,$00,$00
@env18:
	.byte $c9,$cf,$00,$01
@env19:
	.byte $18,$cf,$ce,$cd,$cc,$cb,$ca,$c9,$c8,$c8,$c7,$c7,$c6,$02,$c5,$c5,$c4,$02,$c3,$03,$c2,$c2,$00,$15,$c2,$c2,$c1,$03,$c0,$00,$1c
@env20:
	.byte $0c,$c6,$c7,$c8,$c9,$ca,$ca,$cb,$04,$ca,$00,$09,$ca,$ca,$c9,$02,$c8,$02,$c7,$02,$c6,$02,$c5,$c5,$c4,$c4,$c3,$c2,$c1,$c0,$00,$1d

@samples:
	.byte $11+.lobyte(FAMISTUDIO_DPCM_PTR),$47,$0f,$40,$00 ; 00 fdbass A# (Pitch:15)
	.byte $23+.lobyte(FAMISTUDIO_DPCM_PTR),$45,$0f,$40,$00 ; 01 fdbass B (Pitch:15)
	.byte $35+.lobyte(FAMISTUDIO_DPCM_PTR),$47,$03,$40,$00 ; 02 fdbass C (Pitch:3)
	.byte $35+.lobyte(FAMISTUDIO_DPCM_PTR),$47,$0f,$40,$00 ; 03 fdbass C (Pitch:15)
	.byte $47+.lobyte(FAMISTUDIO_DPCM_PTR),$47,$05,$40,$00 ; 04 fdbass C# (Pitch:5)
	.byte $47+.lobyte(FAMISTUDIO_DPCM_PTR),$47,$07,$40,$00 ; 05 fdbass C# (Pitch:7)
	.byte $47+.lobyte(FAMISTUDIO_DPCM_PTR),$47,$08,$40,$00 ; 06 fdbass C# (Pitch:8)
	.byte $47+.lobyte(FAMISTUDIO_DPCM_PTR),$47,$09,$40,$00 ; 07 fdbass C# (Pitch:9)
	.byte $47+.lobyte(FAMISTUDIO_DPCM_PTR),$47,$0f,$40,$00 ; 08 fdbass C# (Pitch:15)
	.byte $59+.lobyte(FAMISTUDIO_DPCM_PTR),$46,$0f,$40,$00 ; 09 fdbass D (Pitch:15)
	.byte $6b+.lobyte(FAMISTUDIO_DPCM_PTR),$2b,$0e,$40,$00 ; 0a fdkick (Pitch:14)
	.byte $0f+.lobyte(FAMISTUDIO_DPCM_PTR),$38,$0f,$40,$01 ; 0b is a (Pitch:15)
	.byte $2c+.lobyte(FAMISTUDIO_DPCM_PTR),$6d,$0f,$40,$01 ; 0c machine (Pitch:15)
	.byte $48+.lobyte(FAMISTUDIO_DPCM_PTR),$7e,$0b,$40,$01 ; 0d MUSIC_PCM_DS-geo (Pitch:11)
	.byte $00+.lobyte(FAMISTUDIO_DPCM_PTR),$41,$0f,$34,$00 ; 0e snare (Pitch:15)
	.byte $00+.lobyte(FAMISTUDIO_DPCM_PTR),$39,$0f,$40,$01 ; 0f this (Pitch:15)
	.byte $1e+.lobyte(FAMISTUDIO_DPCM_PTR),$37,$0f,$40,$01 ; 10 time (Pitch:15)

@song0ch0:
	.byte $46, $06, $4e, $fe, $4f, $01, $af, $7b, $8c, $38, $85, $3a, $85, $75, $4e, $00, $4f, $02, $8a
@song0ref20:
	.byte $1e, $bd, $4e, $00, $4f, $02, $1e, $bd, $4e, $00, $4f, $02, $1e, $bd, $4e, $00, $4f, $02, $1e, $bd, $4e, $00, $4f, $02
	.byte $41, $08
	.word @song0ref20
@song0ref47:
	.byte $1f, $bd, $1f, $bd, $1e, $bd, $4e, $00, $4f, $02, $1e, $bd
	.byte $41, $08
	.word @song0ref47
	.byte $4e, $00, $4f, $02
	.byte $41, $08
	.word @song0ref20
	.byte $44, $00
@song0ch1:
	.byte $bf, $7d, $49, .lobyte(@env8), .hibyte(@env8), $4f, $01, $8e
@song0ref80:
	.byte $21, $85, $8e, $1e, $85, $1a, $85, $15, $85, $16, $81, $43, $50, $fe, $22, $23, $89, $1a, $85, $19, $85, $18, $81, $43
	.byte $50, $fe, $24, $25, $89, $17, $89, $51, $fe, $70, $8f, $51, $fe, $00
@song0ref118:
	.byte $93, $7d, $26, $85, $23, $85, $1f, $85, $1a, $43, $50, $fb, $26, $28, $83, $1a, $8d, $1f, $85, $1e, $85
@song0ref139:
	.byte $1d, $81, $43, $50, $ff, $29, $2a, $89, $1c, $85, $51, $fe, $70, $8f, $51, $fe, $00, $8f, $7d, $15, $85, $49, .lobyte(@env8)
	.byte .hibyte(@env8), $4f, $01
	.byte $41, $14
	.word @song0ref80
	.byte $51, $fd, $70, $81, $43, $19, $85, $51, $fd, $00, $8b, $83, $00
	.byte $41, $0b
	.word @song0ref118
	.byte $81, $45, $81, $1a, $83, $45, $1a, $81, $45, $81, $1f, $81, $47, $01, $1e, $81
	.byte $41, $08
	.word @song0ref139
	.byte $7d, $00, $81, $47, $01, $1a, $81, $28, $8d, $51, $fe, $50, $97, $7d, $26, $28, $26, $81, $23, $1e, $83, $21, $85, $1c
	.byte $8d, $51, $fe, $50, $95, $47, $01, $7d, $1c, $81, $47, $04, $1e, $83, $23, $81, $47, $04, $21, $81, $47, $06, $1e, $83
	.byte $47, $03, $26, $8f, $51, $fe, $50, $97, $7d, $26, $81, $47, $01, $28, $81, $2a, $81, $47, $01, $28, $81, $2a, $81, $47
	.byte $01, $2d, $81, $2a, $8d, $28, $8d, $45, $85, $21, $47, $04, $26, $47, $04, $23, $81, $21, $81, $47, $02, $1e, $81, $1a
	.byte $43, $50, $fb, $26, $28, $83, $28, $8d, $51, $fe, $50, $97, $7d, $2d, $81, $2a, $81, $28, $81, $26, $81, $23, $81, $26
	.byte $81, $21, $8d, $51, $fe, $50, $97, $7d, $1a, $85, $1c, $85, $26, $43, $50, $f9, $32, $36, $81, $43, $2a, $21, $85, $51
	.byte $fe, $80, $8f, $7d, $51, $fe, $80, $26, $89, $7d, $23, $81, $26, $81, $28, $81, $2a, $81, $28, $81, $2a, $81, $28, $81
	.byte $26, $85, $51, $fe, $90, $8b, $7d, $26, $43, $50, $02, $32, $31, $83, $47, $03, $23, $89, $51, $fe, $40, $97, $7d, $00
	.byte $ad, $90, $12, $85, $13, $85, $15, $8d, $13, $85, $13, $81, $12, $81, $12, $9d, $00, $85, $13, $85, $12, $85, $12, $85
	.byte $10, $85, $12, $85, $10, $85, $0e, $85, $0e, $9d, $00, $9d, $44, $00
@song0ch2:
	.byte $bf, $8a
@song0ref436:
	.byte $26, $bd, $26, $bd, $26, $bd, $2f, $bd
	.byte $41, $08
	.word @song0ref436
	.byte $36, $bd, $36, $bd, $32, $bd, $2f, $ad, $49, .lobyte(@env4), .hibyte(@env4), $36, $89, $49, .lobyte(@env0), .hibyte(@env0)
	.byte $4a, $00, $81, $8a, $36, $bd, $36, $bd, $32, $bd, $2f, $bd
	.byte $41, $08
	.word @song0ref436
	.byte $44, $00
@song0ch3:
	.byte $78, $88, $21, $00, $47, $01, $1f
@song0ref488:
	.byte $00, $73, $80, $22, $81, $47, $01, $78, $88, $1e, $00, $73, $80, $22, $81, $00, $81, $22, $81, $47, $01, $78, $88, $1e
	.byte $00, $73, $80, $22, $81, $47, $01, $78, $88, $1d
	.byte $41, $0b
	.word @song0ref488
	.byte $47, $01, $78, $88, $1f, $00
@song0ref531:
	.byte $7f, $86, $20, $85, $77, $80, $22, $81, $47, $01, $7c, $82, $1e, $81, $7f, $84, $23, $85, $77, $80, $22, $81, $47, $01
	.byte $7c, $82, $1e, $81
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
@song0ref577:
	.byte $7f, $86, $20, $85, $77, $80, $22, $81, $47, $01, $7c, $82, $1e, $81, $7f, $84, $23, $81, $7e, $22, $81, $7d, $20, $85
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $7f, $86, $20, $81, $78, $80, $23, $81, $77, $22, $81, $47, $01, $7c, $82, $20, $81, $7f, $84, $23, $81, $7e, $22, $81
	.byte $7d, $20, $85
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref577
	.byte $7f, $92, $21, $8d, $84, $23, $85, $77, $80, $22, $81, $47, $01, $7c, $82, $1e, $81
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $41, $0c
	.word @song0ref531
	.byte $44, $00
@song0ch4:
	.byte $bf
@song0ref739:
	.byte $0b, $06
@song0ref741:
	.byte $8b, $0f, $00, $83, $06, $85, $0b, $08, $8b, $0f, $07, $8b
	.byte $41, $0e
	.word @song0ref739
@song0ref756:
	.byte $0b, $06, $8b, $0f, $00, $83, $03, $85, $0b, $08, $8b, $0f, $07, $83, $06, $85, $0b, $05, $8b, $0f, $00, $83, $05, $85
	.byte $0b, $00, $87, $06, $81, $0f, $07, $0f, $07, $0f, $06, $83
	.byte $41, $0e
	.word @song0ref739
	.byte $41, $0e
	.word @song0ref739
	.byte $41, $24
	.word @song0ref756
@song0ref801:
	.byte $0b, $08, $87, $08, $81, $0f, $00, $83, $08, $85, $0b, $08, $83, $08, $85, $0f, $07, $83, $06, $85, $0b, $08
	.byte $41, $0b
	.word @song0ref741
	.byte $83, $07, $85
	.byte $41, $24
	.word @song0ref756
	.byte $41, $16
	.word @song0ref801
	.byte $41, $0b
	.word @song0ref741
	.byte $83, $07, $85
	.byte $41, $1f
	.word @song0ref756
	.byte $83, $06, $85
	.byte $41, $09
	.word @song0ref739
	.byte $06, $8b, $0f, $07, $8b
	.byte $41, $09
	.word @song0ref739
	.byte $06, $8b, $0f, $07, $8b
	.byte $41, $09
	.word @song0ref739
	.byte $06, $8b, $0f, $07, $8b
	.byte $41, $09
	.word @song0ref756
	.byte $00, $87, $06, $81, $0f, $07, $83, $06, $85, $44, $00
@song1ch0:
	.byte $46, $01
@song1ch0loop:
	.byte $97, $97, $97, $42
	.word @song1ch0loop
@song1ch3:
@song1ch2:
@song1ch1:
@song1ch1loop:
	.byte $97, $97, $97, $42
	.word @song1ch1loop
@song1ch4:
@song1ch4loop:
	.byte $0f, $0b, $0a, $09, $04, $02, $01, $10, $0c, $11, $0d, $0e, $00, $95, $97, $42
	.word @song1ch4loop
