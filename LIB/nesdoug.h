...
#define multi_vram_buffer_horz(data, len, ppu_address) (xargs[0] = len, storeWordToSreg((uintptr_t)data), __A__ = LSB(ppu_address), __AX__<<=8, __AX__ |= MSB(ppu_address)|NT_UPD_HORZ, _multi_vram_buffer(__EAX__))
// to push multiple writes as one sequential horizontal write to the vram_buffer
#define multi_vram_buffer_vert(data, len, ppu_address) (xargs[0] = len, storeWordToSreg((uintptr_t)data), __A__ = LSB(ppu_address), __AX__<<=8, __AX__ |= MSB(ppu_address)|NT_UPD_VERT, _multi_vram_buffer(__EAX__))
